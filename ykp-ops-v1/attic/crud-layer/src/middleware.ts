...
// Keep cookie name inline ΓÇö do NOT import from session.ts (Node crypto breaks Edge Runtime).
const SESSION_COOKIE = 'ykp_ops_session';
const PUBLIC = ['/login', '/api/auth/login', '/api/auth/logout', '/api/ops/notify/daily-brief'];
// Public read endpoints for Hermez / owner hub integration (GET only;
