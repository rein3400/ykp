...
export const POST = handler(async (req: NextRequest) => {
  ...
  await setSession({
    ...
    investorId: userRow.row.investor_id || undefined
  });

  await logAudit({
    actorUserId: userRow.row.user_id, actorRole: userRow.row.role,
    action: 'login', entity: 'user', entityId: userRow.row.user_id,
    ipAddress: req.headers.get('x-forwarded-for') ?? ''
  }).catch(() => null);

  return ok({
    user_id: userRow.row.user_id, username: userRow.row.username,
    role: (userRow.row.role ?? '').toLowerCase(), investor_id: userRow.row.investor_id,
