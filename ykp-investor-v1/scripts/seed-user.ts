...
async function main(): Promise<void> {
  ...
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: sid, range: `${tab}!A2:A2` });
  if (res.data.values?.[0]?.[0]) { console.log('users tab already seeded'); return; }
  ...
  const row = ['USR-001', username, pw, 'owner', '', 'active', nowTimestampWib(), ''];
  await sheets.spreadsheets.values.append({
    spreadsheetId: sid, range: `${tab}!A2`,
