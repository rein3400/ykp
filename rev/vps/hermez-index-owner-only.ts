 ...
 * OWNER-ONLY AI assistant. Employee features (absen menu, /link, /me,
 * /jadwal, /cuti) live on the HR bot (@justatestermaybot) — non-owners
 * are pointed there.
...
async function handleMessage(msg: TgMessage): Promise<void> {
  ...
  if (!isOwner(msg)) {
    await sendMessage(
      chatId,
      'Maaf, saya hanya melayani owner YKP. 🙏\n\n' +
        'Karyawan: untuk absen, jadwal, dan cuti, gunakan bot HR @justatestermaybot (ketik /start di sana).'
    );
    return;
  }
  try {
...
async function main(): Promise<void> {
  ...
  for (;;) {
    ...
    for (const u of updates) {
      ...
      // callback_query was only used by the employee menu, which now lives
      // on the HR bot — ignore callbacks here.
    }
