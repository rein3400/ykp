...
# Write node script via heredoc (single exec_command, no SFTP)
ssh = paramiko.SSHClient()
ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
ssh.connect('187.77.114.168', username='dev', password='password')

# Write the JS file using a heredoc
write_cmd = f"cat > /home/dev/ykp/ykp-finance-v1/import_rev.js << 'NODEEOF'\n{NODE}\nNODEEOF\n"
stdin, stdout, stderr = ssh.exec_command(write_cmd, timeout=30)
stdout.read()
stderr.read()

# Run node with base64 payload as arg
run_cmd = f"export PATH=/home/dev/.nvm/versions/node/v20.20.2/bin:/usr/bin:/bin:$PATH; cd /home/dev/ykp/ykp-finance-v1 && node import_rev.js '{b64}'"
stdin, stdout, stderr = ssh.exec_command(run_cmd, timeout=120)
print('=== import ===')
