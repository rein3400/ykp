...
# Service account creds (from VPS .env)
...
print('TABS:', [s['properties']['title'] for s in meta['sheets']])

# Read master_outlet
res = sheet.values().get(spreadsheetId=SHEET_ID, range="master_outlet!A1:K50").execute()
rows = res.get('values', [])
print('\n=== master_outlet ===')
for r in rows:
    print(r)
...
# Read master_brand
res = sheet.values().get(spreadsheetId=SHEET_ID, range="master_brand!A1:H20").execute()
rows = res.get('values', [])
print('\n=== master_brand ===')
for r in rows:
    print(r)
