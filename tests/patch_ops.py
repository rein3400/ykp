...
# 1. telegram.ts — replace in-memory linking with Sheets persistence (rowIndex variant)
...
export async function consumeLinkCode(code: string, telegramChatId: string): Promise<string | null> {
  ...
  if (!user) return null;
  await updateRow(TABS.users, user.rowIndex, { ...user.row, telegram_id: telegramChatId }).catch(() => null);
  await updateRow(TABS.telegramLinkCodes, codeRow.rowIndex, {
    ...entry,
...
old = '''  hermezAlerts: 'ops_hermes_alert_log',
  telegramDeliveryLog: 'ops_telegram_delivery_log',
} as const;'''
new = '''  hermezAlerts: 'ops_hermes_alert_log',
  telegramDeliveryLog: 'ops_telegram_delivery_log',
...
patch(f'{base}/src/app/api/ops/telegram/link/route.ts', old, new)
