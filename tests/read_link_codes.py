...
# Read telegram_link_codes to see existing rows + division column
r = svc.spreadsheets().values().get(spreadsheetId=sid, range="telegram_link_codes!A1:G20").execute()
rows = r.get('values', [])
print("telegram_link_codes rows:")
for row in rows:
    print(row)
