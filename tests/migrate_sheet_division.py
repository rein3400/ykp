...
# 1. Fetch service account creds from VPS .env
...
# 2. Read current header
r = svc.spreadsheets().values().get(spreadsheetId=sid, range="telegram_link_codes!A1:H1").execute()
cur = r.get('values', [[]])[0]
print("current header:", cur)

new_header = ['code', 'user_id', 'expires_at', 'telegram_chat_id', 'consumed_at', 'created_at', 'division']
if cur == new_header:
    print("header already correct, no change")
else:
    svc.spreadsheets().values().update(
        spreadsheetId=sid,
        range="telegram_link_codes!A1:G1",
        valueInputOption='RAW',
        body={'values': [new_header]}
    ).execute()
    print("header updated to:", new_header)

# 3. Verify
r = svc.spreadsheets().values().get(spreadsheetId=sid, range="telegram_link_codes!A1:H1").execute()
print("verify header:", r.get('values'))
